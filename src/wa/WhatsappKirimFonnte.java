...
public class WhatsappKirimFonnte extends javax.swing.JDialog {
    ...
    private Connection koneksi = koneksiDBWa.condb();
...
//        TPesan.setWrapStyleWord(true); 
    ...
    private void initComponents() {
        ...
        DTPReg.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "12-06-2024" }));
...
*/
    ...
    private void BtnKirimActionPerformed(java.awt.event.ActionEvent evt) {
        if (NoHp.getText().trim().equals("")) {
            ...
            Valid.textKosong(TPesan, "Isi Pesan");
        } else {
            try {
                String tanggal = (String) DTPReg.getSelectedItem();
                String pesan = TPesan.getText()
                        .replace("{nama}", TPasien.getText())
                        .replace("{rm}", TNoRM.getText())
                        .replace("{antrian}", NoReg.getText())
                        .replace("{poli}", TPoli.getText())
                        .replace("{dokter}", TDokter.getText())
                ...
                String nomorWa = NoHp.getText().replace("+62", "62").replaceFirst("^0", "62") + "@c.us";

                Connection koneksiWA = koneksiDBWa.condb(); 
                PreparedStatement ps = koneksiWA.prepareStatement(
                        "INSERT INTO wa_outbox (NOMOR, NOWA, PESAN, TANGGAL_JAM, STATUS, SOURCE, SENDER, SUCCESS, RESPONSE, REQUEST, TYPE, FILE) "
                        + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                );

                ps.setLong(1, 0);
                ps.setString(2, nomorWa);
                ps.setString(3, pesan);
                ps.setString(4, Valid.SetTgl(DTPReg.getSelectedItem() + "") + " 00:00:00");
                ps.setString(5, "ANTRIAN");
                ps.setString(6, akses.getnamars()); 
                ...
                JOptionPane.showMessageDialog(null, "Pesan berhasil dimasukkan ke antrian gateway WhatsApp.", "Sukses", JOptionPane.INFORMATION_MESSAGE);
                BtnKeluarActionPerformed(evt);
            } catch (Exception e) {
                JOptionPane.showMessageDialog(null, "Gagal mengirim pesan: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
