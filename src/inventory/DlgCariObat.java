...
private void JeniskelasKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_JeniskelasKeyPressed
    ...
    */
    ...
    private void CetakObat(){           
                ...
                try{  
                    ps=koneksi.prepareStatement("select resep_obat.no_resep,resep_obat.tgl_peresepan,resep_obat.jam_peresepan,reg_periksa.kd_pj,resep_obat.no_rawat,pasien.no_rkm_medis,"+
                            "pasien.nm_pasien,resep_obat.kd_dokter,dokter.nm_dokter from resep_obat inner join reg_periksa on resep_obat.no_rawat=reg_periksa.no_rawat inner join pasien on "+
                            "reg_periksa.no_rkm_medis=pasien.no_rkm_medis inner join dokter on resep_obat.kd_dokter=dokter.kd_dokter where resep_obat.no_resep=?");
                    try{
                        ...
                        if(rs.next()){
                            ...
                            Sequel.queryu("delete from temporary_resep where temp37='"+akses.getalamatip()+"'");
                            ps2=koneksi.prepareStatement(
                            "select databarang.kode_brng,databarang.nama_brng,resep_dokter.jml,kodesatuan.satuan,resep_dokter.aturan_pakai from resep_dokter inner join databarang "+
                            "on resep_dokter.kode_brng=databarang.kode_brng inner join kodesatuan on databarang.kode_sat=kodesatuan.kode_sat where resep_dokter.no_resep=? order by databarang.kode_brng");
                            try {
                            ...
                            param.put("propinsirs",akses.getpropinsirs());
                            param.put("tanggal",Valid.SetTgl(rs.getString("tgl_peresepan")+""));
