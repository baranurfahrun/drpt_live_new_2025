...
public class DlgDaftarPermintaanResep extends javax.swing.JDialog {
    ...
    private String bangsal="",aktifkanparsial="no",kamar="",alarm="",DEPOAKTIFOBAT="",
            ...
            NoRawat="",NoRM="",Pasien="",DokterPeresep="",Status="",KodeDokter="",Ruang="",KodeRuang="",rincianobat="",finger="",namakamar="";
        ...
        if(TabPilihRawat.getSelectedIndex()==0){
            if(TabRawatJalan.getSelectedIndex()==0){
                if(akses.getberi_obat()==true){
                    ...
                    //    UPDATE `resep_obat` SET `tgl_penyerahan` = '2023-03-10', `jam_penyerahan` = '05:26:13' WHERE `resep_obat`.`no_resep` = '202303100001'
                    }
                }else{
                    JOptionPane.showMessageDialog(null,"Maaf, Anda tidak punya hak akses untuk mengvalidasi...!!!!");
                    TCari.requestFocus();
                ...
                if(tabMode.getRowCount()==0){
                    ...
                    try {
                        ...
                        ps=koneksi.prepareStatement(
                            ...
                            "where resep_obat.no_resep=? order by databarang.nama_brng");
                        try {
                    ...
                    param.put("norm",NoRM);
                    param.put("lahir",Sequel.cariIsi("select DATE_FORMAT(tgl_lahir,'%d-%m-%Y')as tgl_lahir from pasien where no_rkm_medis=?",NoRM));
                    param.put("umur",Sequel.cariIsi("select umur from pasien where no_rkm_medis=?",NoRM));
                    param.put("alergi",Sequel.cariIsi("select alergi from pemeriksaan_ralan where no_rawat=?",NoRawat));
                    param.put("bb",Sequel.cariIsi("select berat from pemeriksaan_ralan where no_rawat=?",NoRawat));
                    kamar=Sequel.cariIsi("select ifnull(kamar_inap.kd_kamar,'') from kamar_inap where kamar_inap.no_rawat='"+NoRawat+"' order by kamar_inap.tgl_masuk desc limit 1");
                        if(!kamar.equals("")){
                    ...
                    param.put("sep",Sequel.cariIsi("select no_sep from bridging_sep where no_rawat=?",NoRawat));
                    ...
                    param.put("logo",Sequel.cariGambar("select setting.logo from setting"));
                    Valid.MyReportqry("rptLembarObat2Awal.jasper","report","::[ Lembar Pemberian Obat ]::","select * from temporary_resep where temporary_resep.temp37='"+akses.getalamatip()+"' order by temporary_resep.no",param);
                    this.setCursor(Cursor.getDefaultCursor());
                }
            }else if(TabRawatJalan.getSelectedIndex()==1){
                JOptionPane.showMessageDialog(null,"Maaf, silahkan buka Daftar Resep...!!!!");
            ...
            if(TabRawatInap.getSelectedIndex()==0){
                ...
                if(tabMode7.getRowCount()==0){
                    ...
                    try {
                       ...
                       +"detail_permintaan_resep_pulang.no_permintaan from detail_permintaan_resep_pulang inner join databarang on detail_permintaan_resep_pulang.kode_brng=databarang.kode_brng "
                       +"inner join kodesatuan on kodesatuan.kode_sat=databarang.kode_sat WHERE detail_permintaan_resep_pulang.no_permintaan=? order by databarang.nama_brng");
                        ...
                        try {
                            ...
                            while(rs.next()){
                                Sequel.menyimpan("temporary_resep","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,new String[]{
                                    ""+i,rs.getString("nama_brng"),rs.getString("dosis"),rs.getString("jml"),rs.getString("satuan"),"","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","",akses.getalamatip()
                                });
                                i++;
                            }
                        } catch (Exception e) {
    ...
    */
    ...
    private void getData() {
        if(tbResepRalan.getSelectedRow()!= -1){
            ...
            Sequel.cariIsi("select no_sep from bridging_sep where no_rawat=? ",NoSEP,NoRawat);
        }
    }
        ...
        ActionListener taskPerformer = (ActionEvent e) -> {
            if(aktif==true){
                ...
                if(detik.equals("05")){
                    ...
                    if(resepbaru>0){
                        try {
                            music = new BackgroundMusic("./suara/resepelektronik.mp3");
                            music.start();
                        } catch (Exception ex) {
